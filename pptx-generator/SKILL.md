---
name: pptx-generator
description: "Generate, edit, and read PowerPoint presentations in both Chinese and English. Create from scratch with PptxGenJS (cover, TOC, content, section divider, summary slides), edit existing PPTX via XML workflows, extract text with markitdown, and add page transition animations (zoom/dissolve/push/wipe/split/comb/wheel/fade) via python-pptx XML injection. Supports language presets (zh/en) for fonts, typography, capitalization, and cultural conventions. Triggers: PPT, PPTX, PowerPoint, presentation, slide, deck, slides, 加动画, 切换动画, 英文PPT, English presentation."
license: MIT
metadata:
  version: "1.1"
...
| Add transition animations | See [Transitions Reference](references/transitions.md) |
...
| [transitions.md](references/transitions.md) | 页面切换动画效果全览（zoom/dissolve/push/wipe/split/comb/wheel/fade） |
| [language-presets.md](references/language-presets.md) | Language presets for Chinese (zh) and English (en) — fonts, capitalization, date formats, cultural conventions |
...
**Set the language preset first.** Read [language-presets.md](references/language-presets.md) for the correct fonts, capitalization rules, date formats, and cultural conventions. The `lang` parameter is `"zh"` (Chinese, default) or `"en"` (English). This affects fonts, title casing, date format, numbering style, and more.
...
### Step 7b: Add Transition Animations (Optional)

> **Note**: PptxGenJS does NOT natively support slide transitions. This step uses python-pptx XML injection to add PowerPoint-level transitions to the generated `.pptx`.

```bash
python scripts/inject_transitions.py slides/output/presentation.pptx slides/output/presentation_animated.pptx
```

- Default strategy: cover = slow + zoom-in, content = mixed effects, ending = slow + fade
- For custom combos, see [transitions.md](references/transitions.md)
- Delivery file: `presentation_animated.pptx` (the animated version)
...
- `pip install python-pptx lxml` — adding transition animations (optional)
