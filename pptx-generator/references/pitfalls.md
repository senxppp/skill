...
## Transition Animation Pitfalls

1. **PptxGenJS has NO native transition API** — transitions must be injected via python-pptx XML after generation
2. **Animations only visible in PowerPoint/WPS** — browser previews and online viewers won't show them
3. **Use `prs.save()` (python-pptx native)** — never use zipfile manual rewrite + string injection, PowerPoint will refuse to open the file
4. **Don't overuse dramatic effects** — comb, wheel on every page looks gimmicky; fade is the safe default
5. **Cover/ending = slow, content = med** — this is the standard pacing
6. **Max 2-3 effect types per deck** — too many different transitions distracts from content
7. **Round-trip verify after injection** — reopen the output `.pptx` with python-pptx to confirm `{P}transition` elements are present on each slide
