...
> **PptxGenJS does NOT natively support slide transitions.** To add PowerPoint-level transitions (zoom, dissolve, push, wipe, etc.), use the `inject_transitions.py` script after generating the `.pptx`:

```bash
python scripts/inject_transitions.py input.pptx output_animated.pptx
```

See [transitions.md](transitions.md) for the complete reference of available effects and custom combinations.
